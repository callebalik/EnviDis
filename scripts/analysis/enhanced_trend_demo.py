...
from data_visualization import plot_entities_with_documents_histogram, plot_entities_normalized_and_absolute, create_all_plots
...
def demo_enhanced_visualizations():
    ...
    data = generate_sample_data(start_year=1990, end_year=2024, seed=42, generation_type="non_constant_upward")
    print(f"Generated data shape: {data.shape}")
    print(f"Year range: {data.index.min()} - {data.index.max()}")

    # Show basic statistics
    print("\nData Summary:")
    print(f"  Entities range: {data['ObservedEntities'].min()} - {data['ObservedEntities'].max()}")
    print(f"  Documents range: {data['TotalDocuments'].min()} - {data['TotalDocuments'].max()}")
    print(f"  Mean entities per 1000 docs: {(data['ObservedEntities'] / data['TotalDocuments'] * 1000).mean():.2f}")
    ...
    abs_trend_range = absolute_viz['model_results'].fittedvalues.max() - absolute_viz['model_results'].fittedvalues.min()
    norm_trend_range = normalized_viz['normalized_fitted'].max() - normalized_viz['normalized_fitted'].min()
