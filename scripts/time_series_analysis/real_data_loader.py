...
def load_real_time_series(filepath=None, start_date=None, end_date=None):
    ...
    --------
    pd.DataFrame
        DataFrame with Date as index, ObservedEntities and TotalDocuments columns
    """
    if filepath is None:
    ...
    analysis_df = df[['ObservedEntities', 'TotalDocuments']].copy()
    ...
    analysis_df['Days_scaled'] = (analysis_df['Days_since_start'] - analysis_df['Days_since_start'].mean()) / analysis_df['Days_since_start'].std()
    analysis_df['Years_scaled'] = (analysis_df['Years_since_start'] - analysis_df['Years_since_start'].mean()) / analysis_df['Years_since_start'].std()
    analysis_df['Months_scaled'] = (analysis_df['Months_since_start'] - analysis_df['Months_since_start'].mean()) / analysis_df['Months_since_start'].std()
    ...
    print(f"ObservedEntities range: {analysis_df['ObservedEntities'].min()} to {analysis_df['ObservedEntities'].max()}")
    print(f"TotalDocuments range: {analysis_df['TotalDocuments'].min()} to {analysis_df['TotalDocuments'].max()}")
...
def aggregate_to_yearly(data):
    ...
    yearly['Year_scaled'] = (yearly['Year'] - yearly['Year'].mean()) / yearly['Year'].std()
...
def aggregate_to_monthly(data):
    ...
    monthly['Months_scaled'] = (monthly['Months_since_start'] - monthly['Months_since_start'].mean()) / monthly['Months_since_start'].std()
    monthly['Years_scaled'] = (monthly['Years_since_start'] - monthly['Years_since_start'].mean()) / monthly['Years_since_start'].std()
...
def get_data_summary(data):
    ...
    summary = {
        ...
        'date_range': {
            ...
            'end': data.index.max(),
            'span_years': data['Years_since_start'].max() if 'Years_since_start' in data.columns else None
        },
        'observed_entities': {
            'total': data['ObservedEntities'].sum(),
            'mean': data['ObservedEntities'].mean(),
            'median': data['ObservedEntities'].median(),
            'max': data['ObservedEntities'].max(),
            'zero_observations': (data['ObservedEntities'] == 0).sum(),
            'non_zero_percentage': ((data['ObservedEntities'] > 0).sum() / len(data)) * 100
        },
        'total_documents': {
            'total': data['TotalDocuments'].sum(),
            'mean': data['TotalDocuments'].mean(),
            'median': data['TotalDocuments'].median(),
            'max': data['TotalDocuments'].max()
        },
        'co_occurrence_rate': {
            'overall': data['ObservedEntities'].sum() / data['TotalDocuments'].sum() if data['TotalDocuments'].sum() > 0 else 0,
            'per_1000_docs': (data['ObservedEntities'].sum() / data['TotalDocuments'].sum()) * 1000 if data['TotalDocuments'].sum() > 0 else 0
        }
    }
...
def print_data_summary(data):
    ...
    print(f"  Zero observations: {summary['observed_entities']['zero_observations']:,} ({(summary['observed_entities']['zero_observations']/summary['total_observations']*100):.1f}%)")
    print(f"  Non-zero observations: {summary['observed_entities']['non_zero_percentage']:.1f}%")
    ...
    print(f"  Per 1,000 documents: {summary['co_occurrence_rate']['per_1000_docs']:.3f}")
...
if __name__ == "__main__":
    ...
    print(data[['ObservedEntities', 'TotalDocuments', 'Years_since_start']].head(10))
    
    # Show recent observations
    print("\nLast 10 observations:")
    print(data[['ObservedEntities', 'TotalDocuments', 'Years_since_start']].tail(10))
